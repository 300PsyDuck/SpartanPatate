...
double Time::GetTime()
{
	...
	// Section permettant d'obtenir le temps sous un autre système d'exploitation à l'aide de CPP11
	#ifdef OS_OTHER_CPP11
		return std::chrono::duration_cast<std::chrono::nanoseconds>(std::chrono::high_resolution_clock::now() - m_epoch).count() / 1000000000.0;
	#endif

	// Section permettant d'obtenir le temps sous un autre système d'exploitation à l'aide de la librairie SDL
	#ifdef OS_OTHER
